...
function Content({ url }) {
  ...
  const [folderListener, setFolderListener] = useState([])
  ...
  const itemActionRef = useRef()
  const [{ loading, data }, dispatch] = useReducer(contentReducer, {
    loading: true,
    data: folderListener,
  })
  ...
  const renameFile = () => {
    ...
    task
      .rename(inputOptionsRef.current.value, keys)
      .then((resps) => {
        resps.forEach((resp) => {
          // refresh
          setUserDrive({
  ...
  const getFileTitle = (file) => {
    if (file?.data?.name?.length > 46) return `${file.data.name.substr(0, 40)}...${getFileType(file)}`
    else return file?.data?.name
  }
      ...
      return folderListener.map((item, i) => {
        ...
        return (
          <React.Fragment key={i}>
            <ContentItem title={title} preview={preview} item={item} index={i} active={userDrive?.isActive?.indexOf(item.key) !== -1 ? true : false} setItemOptions={setItemOptions} url={url} />
          </React.Fragment>
        )
      })
    } else {
        ...
        <div className="item_options">
          <div className="item_action" ref={itemActionRef}>
            ...
            <button
              ...
              onClick={() => {
                inputOptionsRef.current.value !== "" ? renameFile() : (inputOptionsRef.current.placeholder = "You must type a name")
              }}
            >
              {" "}
              Ok
